...
const VideoGenPage: React.FC = () => {
  ...
  const videoJobs = useMemo(() => itemId ? getJobsByItemIdAndType(itemId, 'video') : [], [itemId, jobs]);
  ...
  const togglePhotoSelection = (jobId: number) => {
  ...
  const handleDescriptionChange = (jobId: number, description: string) => {
    setDescriptions({
      ...descriptions,
      [jobId]: description
    });
                  ...
                  >
                    <div className="flex gap-4">
                      <div className="w-1/4">
                        <div className="relative">
                          ...
                          <button
                            ...
                            className={`absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center ${selectedPhotos.includes(job.id)
                      ...
                      <div className="w-3/4">
                        <textarea
                          rows={3}
                          className={`w-full border rounded-md p-2 ${selectedPhotos.includes(job.id)
